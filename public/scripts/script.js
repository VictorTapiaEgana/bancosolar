...
const nombreUsuario = document.getElementById('nombreCliente');
const balanceUsuario = document.getElementById('balanceCliente');
const generoUsuario = document.getElementById('select');
...
// LLENAR TABLA
async function LlenarTabla(arrayClientes) {

  let fila = '';

  for (const cliente of arrayClientes) {
    
      const Imagen = await ObtenerImagen(cliente.genero);     

      fila += `<tr class="has-text-centered">
                  <td class="px-6 pt-5 pb-4">${cliente.id}</td>
                  <td class="px-6 py-3">
                      <div class="is-flex is-align-items-center">
                          <img class="mr-4 image is-rounded is-cover is-32x32 iconoCliente" src=${Imagen} alt=" icono de ${ cliente.nombre }">
                          <div>
                              <p class="has-text-weight-medium">${cliente.nombre}</p>                                
                          </div>
                      </div>
                  </td>
                  <td class="px-6 pt-5 pb-4">${convertirFecha(cliente.fecha_creacion)}</td>
                  <td class="px-6 pt-5 pb-4">
                      <span class="">${convertirMoneda(cliente.balance)}</span>
                  </td>
                  <td class="px-6 py-4">
                       <button onclick="(editarDatos(${cliente.id},' ${cliente.nombre}', ${cliente.balance}, '${cliente.genero}'))">
                      ...
                      <button class="" onclick="(EliminarDatos(${ cliente.id },'${cliente.nombre}','${Imagen}'))">
...
// SELECCION DE DATOS PARA EDITAR CLIENTE
function editarDatos(id, nombre, balance, genero){
    // alert(`${id}, ${nombre}, ${balance}, ${genero}`)

    btnAgregar.style.display = 'none';
    btnEditar.style.display = 'block';

    nombreUsuario.value =  nombre;
    balanceUsuario.value = balance;
    generoUsuario.value = genero;
    IDEditar = id;

};
// DATOS A ELIMINAR
function EliminarDatos(id , nombre, imagen){

   IDAEliminar = id;
   NombreAEliminar = nombre;
   ImagenAEliminar = imagen;

   SectionModal = document.getElementById('BodyModal')

   const contenido = ` <div id="SectionModal">
                          <img src="${ ImagenAEliminar }" alt="">
                          
                          <div>
                              <label class="label">ID Cliente     : ${ IDAEliminar }</label>
                              <label class="label">Nombre Cliente : ${ NombreAEliminar }</label>
                          </div>
                       </div>  

                        <p id="eliminarRegistro">Desea eliminar este Registro ?</p>`
  
   SectionModal.innerHTML = contenido;
...
btnEditar.addEventListener('click',(e)=>{
  ...
  let EditarNombre  =  nombreUsuario.value.trim();
  let EditarBalance = balanceUsuario.value.trim();
  let EditarGenero = document.getElementById('select').value.trim();
  ...
  try {
    ...
    console.log(`X Error al editar los datos del cliente : `, error);
...
// DAR FORMATO CURRENCY A CAMPO BALANCE
...
// OBTENER IMAGEN EXTERNA SEGUN GENERO
async function ObtenerImagen(genero){  

   const sexo = genero == "Masculino" ? 'male' : 'female';     

  try {  

    const response = await fetch(`https://randomuser.me/api/?gender=${sexo}`);
    const data = await response.json();
    return data.results[0].picture.large;
    
  } catch (error) {

     console.log(`X Error el obtener la imagen : `, error);
...
btnAgregar.addEventListener('click', (e) => {
  ...
  const nombreCliente = nombreUsuario.value.trim();
  const balanceCliente = balanceUsuario.value.trim();
  const generoCliente = document.getElementById('select').value.trim();

  if (generoCliente != 'Seleccion Género' && nombreCliente != '' && balanceCliente != ''){
    ...
    fetch('http://localhost:3010/usuarios', {
      ...
      .catch((error) => {
         ...
         console.error('X Error al insertar el registro : ', error);

      });

  }else{
      document.getElementById('errorCliente').textContent = ' * Todos los campos son obligatorios'
  }
...
function configurarModal() {
      ...
      document.addEventListener('keydown', (event) => {
        ...
        if(event.key === "Escape") {
            ...
            nombreUsuario.value='';
            balanceUsuario.value='';
            generoUsuario.value= 'Seleccion Género';
